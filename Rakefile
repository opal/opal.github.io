...
def pages_branch
  ENV.fetch("PAGES_BRANCH", "master")
end

PROJECT_ROOT = `git rev-parse --show-toplevel`.strip
BUILD_DIR    = File.join(PROJECT_ROOT, "build")
GH_PAGES_REF = File.join(BUILD_DIR, ".git/refs/remotes/#{remote_name}/#{pages_branch}")
...
file GH_PAGES_REF => BUILD_DIR do
  ...
  cd BUILD_DIR do
    ...
    if `git branch -r` =~ /#{pages_branch}/
      sh "git checkout #{pages_branch}"
    else
      sh "git checkout --orphan #{pages_branch}"
      sh "touch index.html"
      sh "git add ."
      sh "git commit -m 'initial #{pages_branch} commit'"
      sh "git push #{remote_name} #{pages_branch}"
    end
  end
end
...
task :sync do
  cd BUILD_DIR do
    sh "git fetch #{remote_name}"
    sh "git reset --hard #{remote_name}/#{pages_branch}"
  end
end
